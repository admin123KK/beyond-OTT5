...
class AddNewDepositController extends GetxController {
  ...
  MainDepositMethodResponseModel depositMethodResponseModel =
  ...
  void changeInfoWidgetValue(double amount) {
    ...
    charge =
        '${StringConverter.twoDecimalPlaceFixedWithoutRounding('$totalCharge')} $currency';
    double payable = totalCharge + amount;
    payableText =
        '${StringConverter.twoDecimalPlaceFixedWithoutRounding('$payable')} $currency';

    rate = double.tryParse(paymentMethod?.rate ?? '0') ?? 0;
    conversionRate = '1 $currency = $rate ${paymentMethod?.currency ?? ''}';
    inLocal = StringConverter.twoDecimalPlaceFixedWithoutRounding(
        '${payable * rate}');
    update();
    return;
  }
  ...
  void submitDeposit(String price) async {
    ...
    InsertDepositResponseModel mo = await depositRepo.insertDeposit(
        subId, amount1, paymentMethod?.methodCode, paymentMethod?.currency);

    if (mo.status != MyStrings.success) {
      List<String> error = [];
      if (mo.status != 'success') {
        List<String>? list =
            mo.message?.error ?? [MyStrings.somethingWentWrong];
        error.addAll(list);
      } else {
      ...
      if (redirectUrl.isEmpty) {
        ...
        CustomSnackbar.showCustomSnackbar(
            errorList: error, msg: [''], isError: false);
      } else {
  ...
  void configureGoogleInAppPayment(
      Map<String, dynamic> result, String price) async {
    String token = '';
    ...
    ResponseModel model = await depositRepo.sendManualPaymentRequest(
        token, methodCode, amount, planId, cardHolderName);

    if (model.statusCode == 200) {
      AuthorizationResponseModel model1 =
          AuthorizationResponseModel.fromJson(jsonDecode(model.responseJson));
      if (model1.status?.toLowerCase() == 'success') {
        CustomSnackbar.showCustomSnackbar(
            errorList: [],
            msg: model1.message?.success ?? [MyStrings.success],
            isError: false);
        Get.offAndToNamed(RouteHelper.paymentHistoryScreen);
      } else {
        CustomSnackbar.showCustomSnackbar(errorList: [
          model1.message?.error.toString() ?? MyStrings.requestFailed
        ], msg: [], isError: true);
      }
    } else {
      CustomSnackbar.showCustomSnackbar(
  ...
  void configureAppleInAppPayment(
      Map<String, dynamic> result, String price) async {
    String token = '';
